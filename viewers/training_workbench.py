...
def _create_train_tab():
    with gr.Row():
        with gr.Column(scale=1):
            pipeline_select = gr.Dropdown(
                choices=["Pipeline A (YOLO)", "Pipeline C (RF-DETR)"],
                value="Pipeline A (YOLO)",
                label="Pipeline 类型",
            )
            ...
            model_select = gr.Dropdown(
                ...
                value="yolov8n.pt",
                label="Pipeline A 基础模型",
            )
            rfdetr_variant = gr.Dropdown(
                choices=["n", "s", "m", "l"],
                value="s",
                label="Pipeline C RF-DETR 模型变体",
                visible=False,
            )
            epochs = gr.Slider(10, 300, value=100, step=10, label="训练轮数")
            batch_size = gr.Slider(1, 32, value=8, step=1, label="批量大小")
            img_size = gr.Dropdown(choices=[320, 480, 512, 640, 960], value=640, label="图片尺寸 (Pipeline C 建议 512)")
            with gr.Row():
    ...
    def on_pipeline_change(pipeline_type):
        if "Pipeline C" in pipeline_type:
            return gr.update(visible=False), gr.update(visible=True)
        else:
            return gr.update(visible=True), gr.update(visible=False)

    pipeline_select.change(
        fn=on_pipeline_change,
        inputs=[pipeline_select],
        outputs=[model_select, rfdetr_variant],
    )

    def start_training(pl, ds, model, variant, ep, bs, isz, sl_en, sl_sz):
        global _training_running, _training_log_lines
        if _training_running:
            return "训练正在进行中", "\n".join(_training_log_lines[-20:])

        _training_log_lines = [f"启动训练: {pl}, epochs={ep}, batch={bs}"]
        ...
        def _train_worker():
            ...
            try:
                ...
                if "Pipeline C" in pl:
                    ...
                    config.training.dataset = ds
                    config.training.epochs = int(ep)
                    config.training.batch_size = int(bs)
                    config.training.resolution = int(isz)
                    config.training.model_variant = variant
                    config.slicer.enabled = sl_en
                    if sl_en:
                        config.slicer.slice_size = int(sl_sz)
                        
                    pipeline = PipelineC(config=config)
                    _training_log_lines.append("开始 Pipeline C 训练...")
                    ...
                    _training_log_lines.append("开始 Pipeline A 训练...")
    ...
    train_btn.click(start_training, [pipeline_select, dataset_dir, model_select, rfdetr_variant, epochs, batch_size, img_size, slice_enabled, slice_size], [train_status, train_log])
...
def _create_test_tab():
    with gr.Row():
        with gr.Column():
            pipeline_select_test = gr.Dropdown(
                choices=["Pipeline A (YOLO)", "Pipeline C (RF-DETR)"],
                value="Pipeline A (YOLO)",
                label="Pipeline 类型",
            )
            model_path = gr.Textbox(label="模型路径", placeholder="runs/train_xxx/weights/best.pt 或者 Pipeline C 的模型路径")
            rfdetr_variant_test = gr.Dropdown(
                choices=["n", "s", "m", "l"],
                value="s",
                label="Pipeline C RF-DETR 模型变体",
                visible=False,
    ...
    def on_pipeline_change_test(pipeline_type):
        if "Pipeline C" in pipeline_type:
            return gr.update(visible=True)
        else:
            return gr.update(visible=False)

    pipeline_select_test.change(
        fn=on_pipeline_change_test,
        inputs=[pipeline_select_test],
        outputs=[rfdetr_variant_test],
    )

    def run_test(pl, img_path, model, variant, conf):
        ...
        try:
            ...
            if "Pipeline C" in pl:
                ...
                config = load_pipeline_c_config("config/pipeline_c.yaml")
                config.training.pretrain_weights = model
                config.training.model_variant = variant
                pipeline = PipelineC(config=config)
                pipeline.initialize()
                
                # Pipeline C 的 process 会返回渲染好的图像
                result_dict, vis_image = pipeline.process(img_path, conf)
                
                lines = [f"检测到 {len(result_dict['detections'])} 个缺陷"]
                for det in result_dict['detections']:
                    lines.append(f"  {det['class_name']}: conf={det['confidence']:.3f}, region={det['bbox']}")
                lines.append(f"\n耗时: {result_dict['processing_time_ms']:.1f} ms")
    ...
    test_btn.click(run_test, [pipeline_select_test, test_image, model_path, rfdetr_variant_test, test_conf], [result_image, result_text])
